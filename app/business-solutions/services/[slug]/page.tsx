...
export async function generateStaticParams() {
  const services: ServicesType[] = getMarkDownData('data/business-solutions/services')
  return services.map((service) => ({
    slug: service.slug,
  }))
}

const businessSolutionServiceDetails = async ({ params }: { params: Promise<{ slug: string }> }) => {
  const slug = (await params).slug
  const service = getMarkDownContent('data/business-solutions/services/', slug)
...
export default businessSolutionServiceDetails
